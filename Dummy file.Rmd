...
table(true = x.test$Class, pred = predict(tune_out_r$best.model, newdata = x.test))
```
The table above shows the correlations between the predictor variable and the remaining variables using the best model gained from the tune() function on the SVM model. 
```{r}
error <- table(class_pred, x.train[,1])
insampleerror<-sum(diag(error))/sum(error)

error_r <- table(class_pred_r, x.train[,1])
insampleerror<-sum(diag(error_r))/sum(error_r)
